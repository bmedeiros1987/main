...
export function exportReport(
  ...
  doc.text('RBAC 117 - Lei 13.475/2017', 14, 33);
  ...
  const metricsData = [
    ['Horas de Voo', `${compliance.metrics.totalFlightHours.toFixed(0)}h`, '80h (max)', 'Art. 33, I - Lei 13.475'],
    ['Horas de Trabalho', `${compliance.metrics.totalDutyHours.toFixed(0)}h`, '176h (max)', 'Art. 41 - Lei 13.475'],
    ['Folgas no Mes', `${compliance.metrics.totalDaysOff} dias`, '10 dias (min)', 'Art. 51 - Lei 13.475'],
    ['Sobreavisos', `${compliance.metrics.totalStandby}`, '8 (max)', 'Art. 43, par. 7 - Lei 13.475'],
    ['Operacoes na Madrugada', `${compliance.metrics.nightOperations}`, '4 por 168h (max)', 'Art. 42 - Lei 13.475'],
  ...
  const timelineData = roster.days.map(day => {
    ...
    } else if (['HSB', 'HSBE', 'ASB'].includes(day.type)) {
      details = `${day.dutyReport || ''} - ${day.dutyDebrief || ''}`;
    }
    return [
      day.date,
      day.dayOfWeek,
  ...
  const gymData = gymRecommendations.map(rec => {
    const status = rec.availability === 'ideal' ? 'IDEAL' : 
                   rec.availability === 'possible' ? 'POSSIVEL' : 'EVITAR';
    return [
      rec.date,
      rec.dayOfWeek,
      status,
  ...
  autoTable(doc, {
    startY: y,
    head: [['Data', 'Dia', 'Status', 'Horario', 'Observacao']],
    body: gymData,
    theme: 'striped',
    headStyles: { fillColor: [27, 42, 74], fontSize: 8 },
    bodyStyles: { fontSize: 7 },
    ...
    didParseCell: (data: any) => {
      if (data.section === 'body' && data.column.index === 2) {
        ...
        } else if (text.includes('POSSIVEL')) {
